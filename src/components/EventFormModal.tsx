...
export const EventFormModal: React.FC<EventFormModalProps> = ({
  ...
  const [organizer, setOrganizer] = useState('Synapse Club — WiDS ACEEC Chapter');
  const [coOrganizer, setCoOrganizer] = useState('ACE Engineering College, CSD Department');
  const [dates, setDates] = useState('');
  const [venue, setVenue] = useState('Computer Science Lab 4 & Central Seminar Hall, ACEEC');
  const [targetAudience, setTargetAudience] = useState('1st and 2nd Year CSD Students & Tech Enthusiasts');
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setOrganizer(initialData.organizer || 'Synapse Club — WiDS ACEEC Chapter');
      ...
      setPrice(0);
      setCapacity(250);
      setTopicsString('Python Programming Fundamentals, Pandas DataFrames, Exploratory Data Analysis (EDA), Matplotlib & Seaborn, Capstone Case Study');
      setIsFlagship(true);
    }
  }, [initialData, isOpen]);
