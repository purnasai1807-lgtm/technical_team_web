...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeAdminTab, setActiveAdminTab] = useState<'events' | 'students' | 'analytics'>('events');
      ...
      {activeAdminTab === 'analytics' && (
        <div className="space-y-6">
          ...
          {analytics?.dailyRegistrations && (
            <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800">
              <h4 className="text-sm font-bold text-white mb-4">Registration Velocity</h4>
              <div className="grid grid-cols-5 gap-2 text-center">
                {analytics.dailyRegistrations.map((d, i) => (
                  <div key={i} className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/40">
                    <span className="text-[11px] text-slate-400 font-mono block">{d.date}</span>
                    <span className="text-lg font-bold text-cyan-300 font-mono mt-1 block">{d.count}</span>
                  </div>
                ))}
              </div>
            </div>
