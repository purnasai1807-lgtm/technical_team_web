...
// Technical Events Catalog: Starts empty; Admin will post technical events
let events: StoredEvent[] = [];

// Student Registrations Database (starts empty at present)
...
let notifications = [
  {
    ...
    message: 'Welcome! Upcoming technical workshops and hackathons will be published directly by the administrator.',
    type: 'announcement',
...
app.get('/api/admin/analytics', requireAdmin, (req: Request, res: Response) => {
  ...
  // Dynamic daily trajectory (past 5 days from live registrations)
  ...
  for (let i = 4; i >= 0; i--) {
    ...
    const count = registrations.filter((r) => r.registeredAt.startsWith(dateStr)).length;
    dailyRegistrations.push({ date: label, count });
  }
